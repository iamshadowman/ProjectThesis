...
for r = 1:numel(kValue)
    ...
    for j=1:z   
        if A(j,10) <= z && A(j,10) > 0        
            h5(A(j,10),j) = -l*sqrt(A(j,2));    
        end
    end
    ...
    for j=1:z    
        if A(j,15)<=z    
            h10(A(j,15),j) = -fj*l*sqrt(A(j,5)+1);   
        end
    end
    ...
    for j=1:z    
        if A(j,17)<=z    
            h12(A(j,17),j) = -fj*l*sqrt(A(j,6)+1);   
        end
    end
    ...
    for j=1:z
        if A(j,18)>0 && A(j,18)<=z   
            h13(A(j,18),j) = -fj*l*sqrt(A(j,6));
        end
    end

    H = h1+h2+h3+h4+h5+h6+h7+h8+h9+h10+h11+h12+h13 ;    % total Hamiltonian matrix
